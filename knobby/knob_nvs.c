...
static int cached_rotation = ROTATION_MODE_OFF;
...
void knob_nvs_init(void)
{
    ...
    if (nvs_open("knobby", NVS_READONLY, &handle) == ESP_OK) {
        ...
        cached_rotation = (rot_val < 0) ? ROTATION_MODE_OFF
                        : (rot_val >= ROTATION_MODE_COUNT) ? ROTATION_MODE_OFF
                                           : rot_val;
...
int nvs_get_rotation(void)
{
    return cached_rotation;
}

void nvs_set_rotation(int value)
{
    cached_rotation = (value < 0) ? ROTATION_MODE_OFF
                                  : (value >= ROTATION_MODE_COUNT) ? ROTATION_MODE_OFF
                                                                   : value;
    settings_dirty = true;
}
...
void settings_save(void)
{
    ...
    if (nvs_open("knobby", NVS_READWRITE, &handle) == ESP_OK) {
        ...
        nvs_set_i8(handle, "rotation", (int8_t)cached_rotation);
